class TicketsController < ApplicationController
  ...
  # GET /tickets/new.xml
  def new
    @ticket = @event.tickets.build(:date_open => Time.now.to_datetime, :date_closed => @event.end)
    respond_to do |format|
  ...
  def update
    ...
    respond_to do |format|
      if @ticket.update_attributes(params[:ticket])
        format.html { redirect_to([@event, @ticket], :notice => 'Ticket was successfully updated.') }
        format.xml  { head :ok }
      else
